...
% HORITA Version
function CG_dE_2 = CG_dE_2(RH, T_air, d2_atmos, T_water, d2_water)
    ...
    C_k = n * (1 - 0.9757) * 1000; % 0.9839 (Cappa et al. 2003); 0.9757 (Merlivat 1978)
    alpha2_VL = 1 / alpha2_LV(T_water);
    % epsilon_k = THETA * (1 - (activity*norm_RH(RH, T_air, T_water)/100)) * C_k;
    epsilon_k = THETA * one_minus_h * C_k;
    epsilon_star = (1 - alpha2_VL)*1000;
    
    CG_dE_2 = ((alpha2_VL * d2_water) - ((1-one_minus_h) * d2_atmos) - (epsilon_star + epsilon_k))/(one_minus_h + (0.001 * epsilon_k));
end
